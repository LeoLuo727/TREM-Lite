...
setInterval(() => {
  ...
  for (const id of _eew_list) {
    ...
    if (now_time - data.eq.time > 240_000) {
      ...
      if (variable.eew_list[data.id].layer.p)
...
setInterval(() => {
  ...
  $("#info-title-box-type").textContent =
    (_eew_list.length > 1 ? `${last_map_count + 1}/${_eew_list.length} ` : "") +
    (!data.status
...
function show_eew(data) {
    ...
    if (data.final)
      ...
      if (!variable.eew_list[data.id].cancel) {
        if (variable.speech_status)
          speech.speak({
            text: `剛才的${
              variable.eew_list[data.id].data.status == 1
                ? "緊急地震速報"
                : "地震速報"
            }被取消了`,
          });
        variable.eew_list[data.id].cancel = true;
        if (checkbox("sound-effects-Update") == true)
        ...
        if (variable.eew_list[data.id].layer.p)
          variable.eew_list[data.id].layer.p.remove();
        const iconElement =
          variable.eew_list[data.id].layer.epicenterIcon.getElement();
        if (iconElement) {
  ...
  if (!variable.eew_list[data.id] || variable.eew_list[data.id].cancel) {
  ...
  } else if (data.serial > variable.eew_list[data.id].data.serial) {
    ...
    if (!variable.eew_list[data.id].data.status && data.status) {
      ...
      if (variable.eew_list[data.id].layer.s)
        variable.eew_list[data.id].layer.s.setLatLng([
          data.eq.lat,
          data.eq.lon,
        ]);
      if (variable.eew_list[data.id].layer.s_fill)
        variable.eew_list[data.id].layer.s_fill.setLatLng([
          data.eq.lat,
          data.eq.lon,
        ]);
    }
    if (data.detail == 1 && variable.eew_list[data.id].data.detail == 0) {
      ...
      variable.eew_list[data.id].layer.epicenterIcon.setIcon(icon);
    variable.eew_list[data.id].data = data;
    variable.eew_list[data.id].layer.epicenterIcon.setLatLng([
      data.eq.lat,
      data.eq.lon,
    ]);
    if (variable.eew_list[data.id].layer.p)
    ...
    if (variable.eew_list[data.id].speech.max < max) {
      ...
      if (!variable.eew_list[data.id].speech.timer)
        variable.eew_list[data.id].speech.timer = setTimeout(() => {
          speech.speak({
            text: `預估最大震度${intensity_list[
              variable.eew_list[data.id].speech.max
            ]
              .replace("⁺", "強")
              .replace("⁻", "弱")}`,
          });
  ...
  variable.eew_list[data.id].eew_intensity_list = eew_area_pga(
    data.eq.lat,
    data.eq.lon,
    data.eq.depth,
    data.eq.mag
...
function ps_wave_dist(depth, time, now) {
  ...
  const _time_table =
    constant.TIME_TABLE[findClosest(constant.TIME_TABLE_OBJECT, depth)];
  let prev_table = null;
